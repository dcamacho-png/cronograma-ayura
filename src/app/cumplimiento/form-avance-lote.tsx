...
export function FormAvanceLote({
  ...
  if (!abierto) {
    return (
      <button
        ...
        className="rounded-lg border border-bosque px-2 py-1 text-xs font-semibold text-bosque hover:bg-arena/40"
      >
        Registrar avance
      </button>
    )
  ...
  return (
    <form action={accion} className="flex w-full flex-col gap-2 rounded-lg border border-borde bg-arena p-2 text-xs">
      ...
      <div className="flex flex-col gap-1">
        <span className="font-semibold text-tinta">Lotes realizados — {etiquetaMedida(unidad)}</span>
        {lotes.map((l) => (
          <div key={l.id} className="flex items-center gap-2">
            ...
            <input name={`cantidad_${l.id}`} type="number" step="any" min="0" placeholder="cantidad" className="w-24 rounded-lg border border-borde bg-marfil p-1 focus:outline-none focus:ring-2 focus:ring-bosque/40" />
          </div>
        ))}
      </div>
